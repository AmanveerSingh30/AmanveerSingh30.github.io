...
export class HeartParticlesComponent implements AfterViewInit, OnDestroy {
  ...
  private initHeartPath(): void {
    ...
    for (let i = 0; i < steps; i++) {
      const t = (i / steps) * PI2;
      // Adjust formula to create a wider heart shape
      const x = centerX + scaleX * Math.pow(Math.sin(t), 3);
      // Adjust Y scaling for better proportions
      const y = centerY - scaleY * (13 * Math.cos(t) - 5 * Math.cos(2*t) - 2 * Math.cos(3*t) - Math.cos(4*t)) / 16;

      this.heartPath.push([x, y]);
    }
  }
  ...
  private initParticles(): void {
    ...
    for (let i = 0; i < this.particleCount; i++) {
      const particles: any[] = [];
      const x = Math.random() * this.canvasWidth * 0.4; // Initialize in left 40% of screen
      const y = Math.random() * this.canvasHeight;

      for (let k = 0; k < this.particleCount; k++) {
        // Color generation - more vibrant for side display
        let hue: number;
        let saturation = 90 + Math.random() * 10; // Very saturated
        let brightness = 60 + Math.random() * 40; // Bright
        let opacity = 0.3 + Math.random() * 0.4; // More opaque

        switch(this.colorScheme) {
          case 'red':
            hue = 350 + Math.random() * 20; // Red range (350-370)
            break;
          case 'blue':
          ...
          case 'monochrome':
            ...
            break;
          default:
            hue = i/this.particleCount * 360; // rainbow
        }
